...
{
  public:
    Semantics(DiagnosticPrinter &DP) : DiagPrinter(DP) {}

    /// Insert into the symbol table.
    /// At the same time checking for redefinitions
    /// and if it happen, then it register the error.
    void InsertToSymTable(const Token &SymName,
                          Type SymType,
    ...
    // clang-format off
    void VisitFunctionParameterDeclaration(const FunctionParameterDeclaration *node) override;
    // clang-format on
