...
MachineOperand *AliveDefinitions::GetAlreadyComputedExpression(MachineInstruction &I)
{
...
static void CopyPropagation(MachineBasicBlock &MBB)
{
    ...
    for (int i = 0; i < (int)InstList.size(); i++)
    {
        ...
        if (Instr.IsLoad())
        {
            ...
            if (!Instr.GetDefine()->IsVirtualReg())
                continue;

            // If the value is not known yet, then it is now
            if (KnownMemoryValues.count(Source->GetSlot()) == 0)
            {
                assert(Instr.GetDefine()->IsVirtualReg());
                KnownMemoryValues[Source->GetSlot()] = Instr.GetDefine()->GetReg();
            }
            ...
            // load is superflous. Register it's definition as renamable.
                ...
                Renamables[Instr.GetDefine()->GetReg()] =
...
static void CSE(MachineBasicBlock &MBB)
{
    ...
    for (int i = 0; i < (int)InstList.size(); i++)
    {
        ...
        {
            Renamables[Instr.GetDefine()->GetReg()] = ACE->GetReg();
        }
        else
        {
...
static void DeadCodeElimination(MachineBasicBlock &MBB)
{
    ...
    for (int i = Instructions.size() - 1; i >= 0; i--)
    {
        if (auto Use1 = Instructions[i].GetNthUse(0);
            Use1 && (Use1->IsVirtualReg() || (Use1->IsMemory() && Use1->IsVirtual())))
            UsedValues.insert(Use1->GetReg());

        if (auto Use2 = Instructions[i].GetNthUse(1);
            Use2 && (Use2->IsVirtualReg() || (Use2->IsMemory() && Use2->IsVirtual())))
        ...
        if (!Instructions[i].IsDefine() || Instructions[i].IsCall() ||
            Instructions[i].GetDefine()->IsRegister())
            continue;
        ...
        if (UsedValues.count(Instructions[i].GetDefine()->GetReg()) == 0)
